          ...
          text: JSON.stringify(
            {
              ...
              limits: {
                ...
                sweep_outbound_budget: SWEEP_OUTBOUND_BUDGET,
              },
              tools: {
                geolink_geocode: "address/name → one place with bounds",
                geolink_reverse_geocode: "lat,lng → address with district/governorate",
                geolink_search_places: "text query (+ optional center) → places, paged as deep as `limit` asks",
                ...
                geolink_sweep_area: "grid-tile an area and merge searches; results_per_point sets depth; dry_run first",
              },
              workflows: [
                ...
                "Dense area missing results: raise geolink_sweep_area results_per_point (re-run dry_run to see the new cost)",
                "More results from one point: geolink_search_places with a larger limit; source_exhausted tells you when there are no more",
              ...
              api_calls_per_tool: {
                geocode: 1,
                reverse_geocode: 1,
                search_places: "ceil((limit+offset)/20), fewer if the area runs out (+1 if `near` is a name, cached)",
                get_directions: "1 (+1 per named endpoint, cached)",
                distance_matrix: "1 (+1 per named location, cached)",
                find_nearest: "1 matrix (+ search calls in search mode, + cached geocodes)",
                sweep_area: "grid_points × ceil(results_per_point/20) (+1 geocode for named areas)",
              },
            },
            null,
            2,
